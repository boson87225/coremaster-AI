                ...
                <div className="fixed inset-0 z-[60] flex flex-col p-4 pt-16 animate-fade-in bg-slate-950/60 backdrop-blur-md">
                    <div className="flex-grow max-w-lg mx-auto w-full shadow-2xl rounded-[2.5rem] overflow-hidden border border-white/10">
                         <ChatInterface 
                            ...
                            initialMessage="您好，我是您的 AI 健身助理。今天有什麼我可以幫您的嗎？"
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const handleRevoked = () => { setApiRevoked(true); setPage('settings'); };
  ...
  useEffect(() => {
    const splashTimer = setTimeout(() => {
        setShowSplash(false);
        const hasSeenIntro = localStorage.getItem('coremaster_intro_seen');
  ...
  const handleIntroClose = () => {
      localStorage.setItem('coremaster_intro_seen', 'true');
      setShowIntro(false);
  };
  ...
  if (showIntro) return <AppIntroSlides onClose={handleIntroClose} />;
